...
class PhysicsInformedNN:
    ...
    def __init__(self, layers_U, layers_V1,layers_V2,layers_V3,layers_F,x_range, f, num_train_tps):
        ...
        # Physics
        self.f1, self.e1, self.e2, self.e3, self.Ux, self.V1x, self.V2x, self.V3x, self.U_xx, self.V1_xx, self.V2_xx, self.V3_xx,  self.V1ex, self.V2ex, self.V3ex, self.Fx, self.F_xx = self.pinn(
            self.xx_tf)
        self.f1, self.e1,self.e2,self.e3,  self.Uf, self.V1f,self.V2f,self.V3f,self.U_xf,self.V1_xf,self.V2_xf,self.V3_xf,self.V1e,self.V2e,self.V3e,self.Ff, self.F_xf= self.pinn(self.xf)
        ...
        self.f1_test,  self.e1_test, self.e2_test,self.e3_test,self.U_test, self.V1_test,self.V2_test,self.V3_test,self.U_x_test,self.V1_x_test,self.V2_x_test,self.V3_x_test,_,_,_,self.F_test,self.F_xtest= self.pinn(self.x_test_tf)
        # 均方误差
        self.loss_U = tf.reduce_mean((self.Ux - self.ux_star_tf) ** 2)
        self.loss_F = tf.reduce_mean((self.Ff - 2*tf.exp(2*self.xf)+self.xf*tf.exp(self.xf**2)*tf.log(tf.exp(2*self.xf))-2*tf.exp(self.xf**2)+2) ** 2)
        self.loss_f =tf.reduce_mean(((self.xf ** 2) * (self.V1f + self.xf * self.V2f + 0.5 * self.xf ** 2 * self.V3f) + self.Ff - self.U_xf) ** 2)
        ...
        self.loss = self.loss_U*100+self.loss_F*10+self.loss_f *1 + self.loss_e1 *1 + self.loss_e2 *1 + self.loss_e3 *1+ self.loss_i * 1
...
#####################初始化神经网络参数
    ...
    def pinn(self,x):
        ...
        f1 = F+(x**2)*(V1+x*V2+0.5*x**2*V3)-U_x
        e1 =V1_x-tf.log(U)
        e2 = V2_x-x*tf.log(U)
        ...
        return f1,e1,e2,e3,U,V1,V2,V3, U_x ,V1_x,V2_x,V3_x,V1e,V2e,V3e, F, F_x
    ...
    def train(self, Xx,Ux, it, printloss):
        ...
        loss_value,loss_value_U,loss_value_F, loss_value_f,loss_value_e1,loss_value_e2,loss_value_e3,loss_value_i= self.sess.run(
            [self.loss, self.loss_U,self.loss_F,self.loss_f,self.loss_e1,self.loss_e2,self.loss_e3,self.loss_i
             ], tf_dict)
        loss_value_array = [loss_value_U*100,loss_value_F*10,loss_value_f*1,loss_value_e1*1,loss_value_e2*1,loss_value_e3*1,loss_value_i*1]
    ...
    def test(self, num_test_tps):
        ...
        return x_test, U_test, V1_test, V2_test,V3_test,f1_test,e1_test,e2_test,e3_test,U_test_x,V1_test_x,V2_test_x,V3_test_x,F_test
...
def ElasImag(nIter = 20000, print_period=1000, plot_period = 1000):
    ...
    layers_F =[1, 20, 20, 20,1]


    Xx, Ux = import_data('UF.txt')

    f = open("loss1_record_V_fan_3.txt", "w")

    num_train_tps =200
    num_test_tps =20

    model = PhysicsInformedNN(layers_U, layers_V1,layers_V2,layers_V3, layers_F,x_range, f, num_train_tps)
    ...
    for it in range(1, nIter+1):
        ...
        if (it%print_period==0):
            ...
            if (it % plot_period == 0 or it == print_period):
                ...
                plt.figure(1)
                x_test, U_test, V1_test,V2_test,V3_test, _,_, _, _, _, _, _,_,F_test= model.test(num_test_tps)
                F_exact = 2*np.exp(2*x_test)-np.exp(x_test**2)*np.log(np.exp(2*x_test))*x_test+2*np.exp(x_test**2)-2.0
                ...
                print(U)
                sio.savemat('U_data_V_fan_3.mat', {'U': U})
