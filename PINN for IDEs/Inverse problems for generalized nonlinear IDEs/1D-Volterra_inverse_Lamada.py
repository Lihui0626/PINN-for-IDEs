...
class PhysicsInformedNN:
    ...
    def __init__(self, layers_U, layers_V1,layers_V2,layers_V3,layers_LM, x_range, f, num_train_tps):
        ...
        # # Total Loss
        self.loss =self.loss_U*10+self.loss_LM*100+ self.loss_f *0.1+ self.loss_e1*1+ self.loss_e2*1 +self.loss_e3*1+self.loss_i*1
...
#####################初始化神经网络参数
    ...
    def train(self, Xx,Ux, it, printloss):
        ...
        loss_value_array = [loss_value_U*10,loss_value_LM*100,loss_value_f*0.1,loss_value_e1*1,loss_value_e2*1,loss_value_e3*1,loss_value_i*1]
...
def ElasImag(nIter = 20000, print_period = 1000, plot_period = 1000):
    ...
    f = open("loss_record_V_Lamada.txt", "w")
    ...
    for it in range(1, nIter+1):
        ...
        if (it%print_period==0):
            ...
            if (it % plot_period == 0 or it == print_period):
                ...
                print(U)
                sio.savemat('U_data_V_Lamada.mat', {'U': U})
