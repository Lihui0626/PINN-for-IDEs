...
class PhysicsInformedNN:
    ...
    def __init__(self, layers_U, layers_V1,layers_V2,layers_V3,layers_LM, x_range, f, num_train_tps):
        ...
        # # Total Loss
        self.loss =self.loss_U*1+self.loss_LM*1+ self.loss_f *1+ self.loss_e1*1+ self.loss_e2*1 +self.loss_e3*1+self.loss_i*1
...
#####################初始化神经网络参数
    ...
    def train(self, Xx,Ux, it, printloss):
        ...
        loss_value_array = [loss_value_U*1,loss_value_LM*1,loss_value_f*1,loss_value_e1*1,loss_value_e2*1,loss_value_e3*1,loss_value_i*1]
...
def import_data(filepath):
    ...
    data =np.loadtxt("U_20.txt", delimiter=None, converters=None, skiprows=0, usecols=None, unpack=False, ndmin=0)
    Xx = data[:, 0:1]
    Ux = data[:,1:2]
    noise =0
    ...
    for it in range(1, nIter+1):
        ...
        if (it%print_period==0):
            ...
            if (it % plot_period == 0 or it == print_period):
                ...
                plt.title(figtopic)
                plt.show()
ElasImag(nIter =9000, print_period =9000, plot_period =9000)