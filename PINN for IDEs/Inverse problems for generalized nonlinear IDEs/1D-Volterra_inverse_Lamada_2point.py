...
def import_data(filepath):
    ...
    data =np.loadtxt("U2.txt", delimiter=None, converters=None, skiprows=0, usecols=None, unpack=False, ndmin=0)
    Xx = data[:, 0:1]
    Ux = data[:,1:2]
    noise =0
    ...
    for it in range(1, nIter+1):
        ...
        if (it%print_period==0):
            ...
            if (it % plot_period == 0 or it == print_period):
                ...
                plt.title(figtopic)
                plt.show()
ElasImag(nIter =60000, print_period =10000, plot_period =10000)