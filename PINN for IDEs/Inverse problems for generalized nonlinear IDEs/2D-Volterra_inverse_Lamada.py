...
class PhysicsInformedNN:
    ...
    def __init__(self, layers_U, layers_V1, layers_V2, layers_V3, layers_V4, layers_V5, layers_W1, layers_W2, layers_W3,
                 layers_W4, layers_W5, layers_LM, x_range, t_range, f, num_train_tps):
        ...
        # Physics
        # Interior (IDE)  #############PDE
        self.gf, self.f1, self.e1, self.e2, self.e3, self.e4, self.e5, self.e6, self.e7, self.e8, self.e9, self.e10, self.Ux, self.V1x, self.V2x, self.V3x, self.V4x, self.V5x, \
        ...
        self.W1_tx, self.W2_tx, self.W3_tx, self.W4_tx, self.W5_tx, self.LMx, self.LM_xx, self.LM_tx = self.pinn(
            self.tx_tf, self.xx_tf)
        ...
        self.W1_tf, self.W2_tf, self.W3_tf, self.W4_tf, self.W5_tf, self.LMf, self.LM_xf, self.LM_tf = self.pinn(
            self.tt, self.xf)
        # chushi
        ...
        self.V1_xi, self.V2_xi, self.V3_xi, self.V4_xi, self.V5_xi, self.W1_ti, self.W2_ti, self.W3_ti, self.W4_ti, self.W5_ti, self.LMi, self.LM_xi, self.LM_ti = self.pinn(
            self.ti, self.xi)
        ...
        self.U_ttb, self.V1_xb, self.V2_xb, self.V3_xb, self.V4_xb, self.V5_xb, self.W1_tb, self.W2_tb, self.W3_tb, self.W4_tb, self.W5_tb, self.LMb, self.LM_xb, self.LM_tb = self.pinn(
            self.tb, self.xb)
        ...
        self.W3_t_test, self.W4_t_test, self.W5_t_test, self.LM_test, self.LM_x_test, self.LM_t_test = self.pinn(
            self.t_test_tf, self.x_test_tf)
        ...
        self.loss_f = tf.reduce_mean((self.gf + self.tt ** 5 *(self.V1f - self.V2f + self.V3f - self.V4f + self.V5f) + self.U_xf - self.U_tf - self.LMf *self.Uf - self.U_ttf) ** 2)
        ...
        # # Total Loss
        self.loss = self.loss_U * 1+ self.loss_LM * 10 + self.loss_f * 0.1 + self.loss_e1 * 1 + self.loss_e2 * 1 + \
                    self.loss_e3 * 1000 + self.loss_e4 * 100 + self.loss_e5 * 10000 + self.loss_e6 * 0.1 + self.loss_e7 * 0.1 + \
                    self.loss_e8 * 0.1 + self.loss_e9 * 0.1 + self.loss_e10 * 0.1 + self.loss_i * 1 + self.loss_b1 * 1 + \
                    self.loss_b2 * 1
    ...
    def pinn(self, t, x):
        ...
        e9 = W4_t - t ** 6 * U**2
        e10 = W5_t - t ** 8 * U**2
        return gf, f1, e1, e2, e3, e4, e5, e6, e7, e8, e9, e10, U, V1, V2, V3, V4, V5, W1, W2, W3, W4, W5, U_x, U_t, U_tt, V1_x, V2_x, V3_x, V4_x, V5_x, W1_t, W2_t, W3_t, W4_t, W5_t, LM, LM_x, LM_t

    def generateTrain(self, num_train_tps):
        tt = tf.linspace(np.float32(self.t_range[0]), np.float32(self.t_range[1]), 200)
    ...
    def train(self, tx, xx, Ux, it, printloss):
        ...
        loss_value_e8, loss_value_e9, loss_value_e10, loss_value_i, loss_value_b1, loss_value_b2 = self.sess.run(
            [self.loss, self.loss_U, self.loss_LM, self.loss_f, self.loss_e1, self.loss_e2, self.loss_e3, self.loss_e4,
             self.loss_e5, self.loss_e6, self.loss_e7,
             self.loss_e8, self.loss_e9, self.loss_e10, self.loss_i, self.loss_b1, self.loss_b2
             ], tf_dict)
        loss_value_array = [loss_value_U * 1, loss_value_LM * 10, loss_value_f * 0.1, loss_value_e1 * 1,
                            loss_value_e2 * 1, loss_value_e3 * 1000, loss_value_e4 * 100, loss_value_e5 * 10000,
                            loss_value_e6 * 0.1, loss_value_e7 * 0.1, loss_value_e8 * 0.1, loss_value_e9 * 0.1,
                            loss_value_e10 * 0.1,
    ...
    def test(self, num_test_tps):
        ...
        return X_test, U_test, V1_test, V2_test, V3_test, V4_test, V5_test, W1_test, W2_test, W3_test, W4_test, W5_test, LM_test, f1_test, e1_test, e2_test, e3_test, e4_test, e5_test, e6_test, e7_test, e8_test, e9_test, e10_test
...
def ElasImag(nIter=50000, print_period=2000, plot_period=2000):
    ...
    f = open("loss_record_2Dinverse.txt", "w")
    ...
    model = PhysicsInformedNN(layers_U, layers_V1, layers_V2, layers_V3, layers_V4, layers_V5, layers_W1, layers_W2,
                              layers_W3, layers_W4, layers_W5, layers_LM, x_range, t_range, f, num_train_tps)
    ...
    for it in range(1, nIter + 1):
        ...
        if (it % print_period == 0):
            ...
            if (it % plot_period == 0 or it == print_period):
                ...
                X_test, U_test, V1_test, V2_test, V3_test, V4_test, V5_test, W1_test, W2_test, W3_test, W4_test, W5_test, LM_test, f_test, e1_test, e2_test, e3_test, e4_test, e5_test, e6_test, e7_test, e8_test, e9_test, e10_test = model.test(
                    num_test_tps)
                ...
                plt.title(figtopic)
                plt.show()
                sio.savemat('LM_test2D_data_2D_INVERSE.mat', {'LM_test2D': LM_test2D})
                sio.savemat('U_test2D_INVERSE.mat', {'U_test2D': U_test2D})
                sio.savemat('U_err2D_INVERSE.mat', {'U_err2D': U_err2D})
                sio.savemat('X_data_2D_INVERSE.mat', {'X': X_test})
