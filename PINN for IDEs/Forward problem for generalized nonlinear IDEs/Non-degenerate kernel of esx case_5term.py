...
class PhysicsInformedNN:
    ...
    def __init__(self, layers_U, layers_V1,layers_V2,layers_V3,layers_V4,layers_V5,x_range, f, num_train_tps):
        ...
        # Physics
        self.f1, self.e1,self.e2,self.e3,self.e4,self.e5,  self.Uf, self.V1f,self.V2f,self.V3f,self.V4f,self.V5f,self.U_xf,self.V1_xf,self.V2_xf,self.V3_xf,self.V4_xf,self.V5_xf,self.V1e,self.V2e,self.V3e,self.V4e,self.V5e= self.pinn(self.xf)
        ...
        self.f1_test,  self.e1_test, self.e2_test,self.e3_test,self.e4_test,self.e5_test,self.U_test, self.V1_test,self.V2_test,self.V3_test,self.V4_test,self.V5_test,self.U_x_test,self.V1_x_test,self.V2_x_test,self.V3_x_test,self.V4_x_test,self.V5_x_test,_,_,_,_,_= self.pinn(self.x_test_tf)
        # 均方误差
        self.loss_f = tf.reduce_mean((self.U_xf-self.xf**2*self.V1f-self.xf*self.xf**2*self.V2f-0.5*self.xf**2*self.xf**2*self.V3f-self.xf**2*self.xf**3*self.V4f*1/24-self.xf**2*self.xf**4*self.V4f*1/120-2*tf.exp(2*self.xf)+self.xf*tf.exp(self.xf**2)*tf.log(tf.exp(2*self.xf))-2*tf.exp(self.xf**2)+2)** 2)
        ...
        self.loss_e4 = tf.reduce_mean((self.V4_xf - self.xf ** 3 * tf.log(self.Uf)) ** 2)
        self.loss_e5 = tf.reduce_mean((self.V5_xf - self.xf ** 4 * tf.log(self.Uf)) ** 2)
        ...
        self.loss = self.loss_f *0.1 + self.loss_e1 *0.1 + self.loss_e2 *1 + self.loss_e3 *1+ self.loss_e4 *1+ self.loss_e5 *1+ self.loss_i * 1
...
#####################初始化神经网络参数
    ...
    def pinn(self,x):
        ...
        V4_x = tf.gradients(V4, x)  # dv/dx
        V5_x = tf.gradients(V5, x)  # dv/dx

        f1 = U_x-x**2*V1-x*x**2*V2-0.5*x**2*x**2*V3-2*tf.exp(2*x)+tf.exp(x**2)*tf.log(tf.exp(2*x))*x-2*tf.exp(x**2)+2.0
        e1 =V1_x-tf.log(U)
        e2 = V2_x-x*tf.log(U)
        e3 =V3_x -x**2* tf.log(U)
        e4 = V3_x - x ** 3 * tf.log(U)
        e5 = V3_x - x ** 4 * tf.log(U)
    ...
    def train(self, xx,  it, num_train_tps):
        ...
        loss_value_array = [loss_value_f*0.1,loss_value_e1*0.1,loss_value_e2*1,loss_value_e3*1,loss_value_e4*1,loss_value_e5*1,loss_value_i*1]
    ...
    def test(self, num_test_tps):
        ...
        V4_test_x = tf.gradients(V4_test, x_test)  # du/dx
        V5_test_x = tf.gradients(V5_test, x_test)  # du/dx
    ...
    for it in range(1, nIter+1):
        ...
        if (it%print_period==0):
            ...
            if (it % plot_period == 0 or it == print_period):
                ...
                print(U)
                sio.savemat('U_data_V_5.mat', {'U': U})
