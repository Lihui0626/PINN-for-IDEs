...
class PhysicsInformedNN:
    ...
    def __init__(self, layers_U, layers_V1,layers_V2,layers_V3, layers_V4,layers_V5,x_range, f, num_train_tps):
        ...
        # Physics
        self.f1, self.e1, self.e2, self.e3, self.e4, self.e5, self.Uf, self.V1f, self.V2f, self.V3f, self.V4f, self.V5f, self.U_xf, self.V1_xf, self.V2_xf, self.V3_xf, self.V4_xf, self.V5_xf, self.V1e, self.V2e, self.V3e, self.V4e, self.V5e = self.pinn(
            self.xf)
        _, _, _, _, _, _, self.Ui, self.V1i, self.V2i, self.V3i, self.V4i, self.V5i, _, _, _, _, _, _, _, _, _, _, _ = self.pinn(
            self.xi)
        _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, self.V1e, self.V2e, self.V3e, self.V4e, self.V5e = self.pinn(
            self.xe)
        self.f1_test, self.e1_test, self.e2_test, self.e3_test, self.e4_test, self.e5_test, self.U_test, self.V1_test, self.V2_test, self.V3_test, self.V4_test, self.V5_test, self.U_x_test, self.V1_x_test, self.V2_x_test, self.V3_x_test, self.V4_x_test, self.V5_x_test, _, _, _, _, _ = self.pinn(
            self.x_test_tf)
        # 均方误差
        self.loss_f = tf.reduce_mean((self.U_xf -  self.V1e + 0.5 * self.xf ** 2
                                      * self.V2e - self.xf ** 4 * self.V3e * 1 / 24
                    +self.xf**6*self.V4e*1/720-self.xf**8*self.V5e*1/40320- tf.cos(self.xf)) ** 2)
        ...
        self.loss_e4 = tf.reduce_mean((self.V4_xf - self.xf ** 6 * tf. sin(self.Uf)) ** 2)
        self.loss_e5 = tf.reduce_mean((self.V5_xf - self.xf ** 8 *  tf.sin(self.Uf)) ** 2)
        self.loss_i = tf.reduce_mean((self.Ui + 1) ** 2) + tf.reduce_mean((self.V1i-0) ** 2) \
                      ...
                      + tf.reduce_mean((self.V4i-0) ** 2) + tf.reduce_mean((self.V5i-0) ** 2)

        # Total Loss
        self.loss = self.loss_f *1+ self.loss_e1*1+ self.loss_e2*1 +self.loss_e3*0.01+self.loss_e4*0.01+self.loss_e5*0.01+self.loss_i*1
...
#####################初始化神经网络参数
    ...
    def pinn(self,x):
        ...
        f1 = U_x -  V1e + 0.5 * x ** 2  * V2e - x ** 4 *  V3e * 1 / 24 +x ** 6  * V4e * 1 / 720-x ** 8  * V5e * 1 / 40320- tf.cos(x)
        e1 = V1_x -tf.sin(U)
        e2 = V2_x - x ** 2 * tf.sin(U)
        e3 = V3_x - x ** 4 * tf.sin(U)
        e4 = V4_x - x ** 6 * tf.sin(U)
        e5 = V5_x - x ** 8 * tf.sin(U)

        return f1,e1,e2,e3,e4,e5,U,V1,V2,V3, V4,V5, U_x ,V1_x,V2_x,V3_x,V4_x,V5_x,V1e,V2e,V3e,V4e,V5e
    ...
    def train(self, xx,  it, num_train_tps):
        ...
        loss_value_array = [loss_value_f*1,loss_value_e1*1,loss_value_e2*1,loss_value_e3*0.01,loss_value_e4*0.01,loss_value_e5*0.01,loss_value_i*1]
    ...
    def test(self, num_test_tps):
        ...
        return x_test, U_test, V1_test, V2_test,V3_test, V4_test,V5_test,f1_test,e1_test,e2_test,e3_test,e4_test,e5_test,U_test_x,V1_test_x,V2_test_x,V3_test_x,V4_test_x,V5_test_x


def ElasImag(nIter = 40000, print_period = 10000, plot_period = 10000):
    ...
    f = open("loss_record_F_5.txt", "w")

    num_train_tps =100
    num_test_tps =5

    model = PhysicsInformedNN(layers_U, layers_V1, layers_V2, layers_V3, layers_V4, layers_V5, x_range, f,
                              num_train_tps)
    ...
    for it in range(1, nIter + 1):
        ...
        if (it % print_period == 0):
            ...
            if (it % plot_period == 0 or it == print_period):
                ...
                plt.figure(1)
                x_test, U_test, V1_test, V2_test, V3_test, V4_test, V5_test, _, _, _, _, _, _, _, _, _, _, _, _ = model.test(
                    num_test_tps)
                u_exact = np.sin(x_test)
                # UV_test = np.hstack((U_test, V1_test, V2_test, V3_test))
                U = np.hstack((x_test, u_exact, U_test))
                print(U)
                sio.savemat('U_data_F_5.mat', {'U': U})
...
ElasImag(nIter=100000, print_period=20000, plot_period=20000)