...
class PhysicsInformedNN:
    ...
    def __init__(self, layers_U, layers_V1,layers_V2, x_range, f, num_train_tps):
        ...
        # Fredholm积分微分方程
        # 均方误差
        self.loss_f = tf.reduce_mean((self.U_xf -  self.V1e + 0.5 * self.xf ** 2  * self.V2e  - tf.cos(
            self.xf)) ** 2)
        ...
        # Total Loss
        self.loss = self.loss_f *0.1+ self.loss_e1*1+ self.loss_e2*1 +self.loss_i*1
...
#####################初始化神经网络参数
    ...
    def pinn(self,x):
        ...
        f1 = U_x -  V1e + 0.5 * x ** 2 * V2e  - tf.cos(x)
        e1 = V1_x -  tf.sin(U)
    ...
    def train(self, xx,  it, num_train_tps):
        ...
        loss_value_array = [loss_value_f*0.1,loss_value_e1*1,loss_value_e2*1,loss_value_i*1]
    ...
    for it in range(1, nIter+1):
        ...
        if (it%print_period==0):
            ...
            if (it % plot_period == 0 or it == print_period):
                ...
                print(U)
                sio.savemat('U_data_F_2.mat', {'U': U})
