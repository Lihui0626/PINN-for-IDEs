...
class PhysicsInformedNN:
    ...
    def __init__(self, layers_U, layers_V1,layers_V2,layers_V3, x_range, f, num_train_tps):
        ...
        # Physics
        self.gf,self.f, self.e1,self.e2,self.e3, self.Uf, self.V1f,self.V2f,self.V3f,self.U_xf,self.V1_xf,self.V2_xf,self.V3_xf,self.V1e,self.V2e,self.V3e= self.pinn(self.xf)
        _,_,_,_,_,self.Ui, self.V1i,self.V2i,self.V3i,_,_,_,_,_,_,_= self.pinn(self.xi)
        _,_,_,_,_,_,_,_,_,_,_,_,_,self.V1e,self.V2e,self.V3e= self.pinn(self.xe)
        _,self.f_test,  self.e1_test, self.e2_test,self.e3_test,self.U_test, self.V1_test,self.V2_test,self.V3_test,self.U_x_test,self.V1_x_test,self.V2_x_test,self.V3_x_test,_,_,_= self.pinn(self.x_test_tf)
        # 均方误差
        self.loss_f = tf.reduce_mean((self.U_xf - self.xf *self.V1e + self.xf ** 3 * self.V2e/6 - self.xf ** 5 *  self.V3e * 1 / 120 - self.gf) ** 2)
        self.loss_e1 = tf.reduce_mean((self.V1_xf -  self.xf *self.Uf) ** 2)
        self.loss_e2 = tf.reduce_mean((self.V2_xf - self.xf ** 3 * self.Uf) ** 2)
        self.loss_e3 = tf.reduce_mean((self.V3_xf - self.xf ** 5 * self.Uf) ** 2)
        self.loss_i = tf.reduce_mean((self.Ui+1) ** 2) + tf.reduce_mean((self.V1i) ** 2) \
                      + tf.reduce_mean((self.V2i) ** 2) + tf.reduce_mean((self.V3i) ** 2)
        # Total Loss
        self.loss = self.loss_f *1+ self.loss_e1*1+ self.loss_e2*1 +self.loss_e3*1+self.loss_i*1
...
#####################初始化神经网络参数
    ...
    def pinn(self,x):
        ...
        gf =-w*(tf.sin(w*x)+2*tf.sin(2*w*x))
        f = U_x -x* V1e + x ** 3 * V2e /6- x ** 5 * V3e * 1 / 120 - gf
        e1 = V1_x -x * U
        e2 = V2_x - x ** 3 * U
        e3 = V3_x - x ** 5 * U

        return gf,f,e1,e2,e3,U,V1,V2,V3, U_x ,V1_x,V2_x,V3_x,V1e,V2e,V3e
    ...
    def train(self, xx,  it, num_train_tps):
        ...
        loss_value, loss_value_f,loss_value_e1,loss_value_e2,loss_value_e3,loss_value_i= self.sess.run(
            [self.loss, self.loss_f,self.loss_e1,self.loss_e2,self.loss_e3,self.loss_i], tf_dict)
        loss_value_array = [loss_value_f,loss_value_e1,loss_value_e2,loss_value_e3,loss_value_i]
    ...
    def test(self, num_test_tps):
        ...
        return x_test, U_test, V1_test, V2_test,V3_test,f_test,e1_test,e2_test,e3_test,U_test_x,V1_test_x,V2_test_x,V3_test_x


def ElasImag(nIter = 30000, print_period = 1000, plot_period = 10000):
    ...
    layers_V1 =[1, 40, 40,40,1]
    layers_V2 =[1, 40, 40,40,1]
    layers_V3 = [1, 40, 40, 40,1]
    ...
    for it in range(1, nIter+1):
        ...
        if (it%print_period==0):
            ...
            if (it % plot_period == 0 or it == print_period):
                ...
                u_exact = np.cos(w*x_test)+np.cos(2*w*x_test)
                ...
                # print("{:.6f}".format(x_test[0][0]),"{:.6f}".format(u_exact[0][0]),"{:.6f}".format(U_test[0][0]))
                plt.figure()
...
ElasImag(nIter =30000, print_period =10000, plot_period =10000)