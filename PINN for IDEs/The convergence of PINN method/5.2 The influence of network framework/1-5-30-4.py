...
class PhysicsInformedNN:
    ...
    def __init__(self, layers_UV,x_range, f, num_train_tps):
        ...
        # Physics
        self.f1, self.e1, self.e2, self.e3, self.Uf, self.V1f, self.V2f, self.V3f, self.U_xf, self.V1_xf, self.V2_xf, self.V3_xf = self.pinn(
            self.xf)
        _, _, _, _, self.Ui, self.V1i, self.V2i, self.V3i, _, _, _, _ = self.pinn(self.xi)

        self.f1_test, self.e1_test, self.e2_test, self.e3_test, self.U_test, self.V1_test, self.V2_test, self.V3_test, self.U_x_test, self.V1_x_test, self.V2_x_test, self.V3_x_test = self.pinn(
            self.x_test_tf)
        # 均方误差
        self.loss_f = tf.reduce_mean((
                                                 self.U_xf - self.xf ** 2 * self.V1f - self.xf * self.xf ** 2 * self.V2f - 0.5 * self.xf ** 2 * self.xf ** 2 * self.V3f - 2 * tf.exp(
                                             2 * self.xf) + self.xf * tf.exp(self.xf ** 2) * tf.log(
                                             tf.exp(2 * self.xf)) - 2 * tf.exp(self.xf ** 2) + 2) ** 2)
        ...
        self.loss = self.loss_f * 0.1 + self.loss_e1 * 1 + self.loss_e2 * 0.1 + self.loss_e3 * 0.1 + self.loss_i * 1
...
#####################初始化神经网络参数
    ...
    def pinn(self,x):
        ...
        f1 = U_x - x ** 2 * V1 - x * x ** 2 * V2 - 0.5 * x ** 2 * x ** 2 * V3 - 2 * tf.exp(2 * x) + tf.exp(
            x ** 2) * tf.log(tf.exp(2 * x)) * x - 2 * tf.exp(x ** 2) + 2.0
        e1 = V1_x - tf.log(U)
        e2 = V2_x - x * tf.log(U)
        e3 = V3_x - x ** 2 * tf.log(U)

        return f1, e1, e2, e3, U, V1, V2, V3, U_x, V1_x, V2_x, V3_x
    ...
    def train(self, xx,  it, num_train_tps):
        ...
        loss_value_array = [loss_value_f*1,loss_value_e1*1,loss_value_e2*1,loss_value_e3*1,loss_value_i*1]
...
def ElasImag(nIter = 20000, print_period=1000, plot_period = 1000):
    ...
    # Network Structure
    layers_UV =[1, 30, 30,30,30,30, 4]


    Xx = np.linspace(x_range[0], x_range[1], 100)
    Xx = np.reshape(Xx, [-1, 1])

    f = open("loss1_record_net3.txt", "w")
    ...
    for it in range(1, nIter + 1):
        ...
        if (it % print_period == 0):
            ...
            if (it % plot_period == 0 or it == print_period):
                ...
                print(U)
                sio.savemat('U_data_V_net3.mat', {'U': U})
