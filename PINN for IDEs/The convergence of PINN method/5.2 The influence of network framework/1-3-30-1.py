...
class PhysicsInformedNN:
    ...
    def __init__(self, layers_U, layers_V1,layers_V2,layers_V3,x_range, f, num_train_tps):
        ...
        # 均方误差
        self.loss_f = tf.reduce_mean((self.U_xf-self.xf**2*self.V1f-self.xf*self.xf**2*self.V2f-0.5*self.xf**2*self.xf**2*self.V3f-2*tf.exp(2*self.xf)+self.xf*tf.exp(self.xf**2)*tf.log(tf.exp(2*self.xf))-2*tf.exp(self.xf**2)+2)** 2)
...
#####################初始化神经网络参数
    ...
    def train(self, xx,  it, num_train_tps):
        ...
        loss_value_array = [loss_value_f*0.1,loss_value_e1*1,loss_value_e2*0.1,loss_value_e3*0.1,loss_value_i*1]
...
def ElasImag(nIter = 20000, print_period=1000, plot_period = 1000):
    ...
    f = open("loss1_record_net1.txt", "w")
    ...
    for it in range(1, nIter+1):
        ...
        if (it%print_period==0):
            ...
            if (it % plot_period == 0 or it == print_period):
                ...
                print(U)
                sio.savemat('U_data_V_3.mat', {'U': U})
