...
class PhysicsInformedNN:
    ...
    def __init__(self, layers_U, layers_V1,layers_V2,layers_V3, x_range, f, num_train_tps):
        ...
        # # 平均绝对误差
        # self.loss_f = tf.reduce_mean(abs(self.U_xf -  self.V1e + 0.5 * self.xf ** 2  * self.V2e - self.xf ** 4  * self.V3e * 1 / 24 - tf.cos(self.xf)))
        # self.loss_e1 = tf.reduce_mean(abs(self.V1_xf -tf.sin(self.Uf)))
        # self.loss_e2 = tf.reduce_mean(abs(self.V2_xf - self.xf**2*tf.sin(self.Uf)))
        # self.loss_e3 = tf.reduce_mean(abs(self.V3_xf -self.xf**4*tf.sin(self.Uf)))
        ...
        # 均方根误差
        self.loss_f =tf.sqrt(tf.reduce_mean((self.U_xf -  self.V1e + 0.5 * self.xf ** 2  * self.V2e - self.xf ** 4  * self.V3e * 1 / 24 - tf.cos(self.xf)) ** 2))
        self.loss_e1 =tf.sqrt(tf.reduce_mean((self.V1_xf - tf.sin(self.Uf)) ** 2))
        self.loss_e2 =tf.sqrt(tf.reduce_mean((self.V2_xf - self.xf ** 2 * tf.sin(self.Uf)) ** 2))
        self.loss_e3 =tf.sqrt(tf.reduce_mean((self.V3_xf - self.xf ** 4 * tf.sin(self.Uf)) ** 2))
